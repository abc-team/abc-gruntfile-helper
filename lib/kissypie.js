...
var KISSY_PIE = {
    ...
    parse: function( grunt, defaults ){
        ...
        this.grunt = grunt;
        this.taskName = Utils.getTaskName();
    ...
    taskInit: function( grunt ){
        ...
        if( pageInfoList.length > 1 && this.taskName == 'watch' ){
            ...
            grunt.task.registerTask( 'watch', function(){
                ...
                pageInfoList.forEach(function( page, index ){

                    var args = [ 'watch' ].concat( page.args );

                    // 添加额外的逻辑，对于common只watch一次，对于其他watch线程，添加--no-common参数以标识不进行common的watch
                    if( index > 0 ){
                        args.push( '--no-common' );
                    }
            ...
            // 若添加了 no-common 标记，则不watch common
            var withoutCommon = grunt.option( 'no-common' );
